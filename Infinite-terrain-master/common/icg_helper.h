...
// compiles the vertex, geometry and fragment shaders stored in the given
// strings
inline GLuint CompileShaders(const char *vshader, const char *fshader,
  ...
  if (tcshader != NULL) {
    ...
    glGetShaderiv(tessellation_control_shader_id, GL_INFO_LOG_LENGTH,
                  &info_log_length);
    if (!success) {
      vector<char> tessellation_control_shader_error_message(info_log_length);
      glGetShaderInfoLog(tessellation_control_shader_id, info_log_length, NULL,
  ...
  if (teshader != NULL) {
    ...
    // check Shader
    glGetShaderiv(tessellation_evaluation_shader_id, GL_COMPILE_STATUS,
                  &success);
    glGetShaderiv(tessellation_evaluation_shader_id, GL_INFO_LOG_LENGTH,
                  &info_log_length);
    if (!success) {
      vector<char> tessellation_evaluation_shader_error_message(
          info_log_length);
  ...
  if (tcshader != NULL)
    glAttachShader(program_id, tessellation_control_shader_id);
  if (teshader != NULL)
    glAttachShader(program_id, tessellation_evaluation_shader_id);
  if (gshader != NULL)
...
inline GLuint LoadShaders(const char *vertex_file_path,
  ...
  string vertex_shader_code, fragment_shader_code, geometry_shader_code;
  {
    ...
    if (vertex_shader_stream.is_open()) {
      vertex_shader_code =
          string(istreambuf_iterator<char>(vertex_shader_stream),
    ...
    if (fragment_shader_stream.is_open()) {
      fragment_shader_code =
          string(istreambuf_iterator<char>(fragment_shader_stream),
                 istreambuf_iterator<char>());
    ...
    if (geometry_file_path != NULL) {
      ...
      if (geometry_shader_stream.is_open()) {
        geometry_shader_code =
            string(istreambuf_iterator<char>(geometry_shader_stream),
                   istreambuf_iterator<char>());
